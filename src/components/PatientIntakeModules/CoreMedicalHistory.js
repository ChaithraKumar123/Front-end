...
class CoreMedicalHistory extends Component {
 ...
 comparer = (otherArray) => {
  ...
  onChangeMultipleSelect = (input) => async (event) => {
    let todel;

    try{
      if (event != null) 
      {
        // var onlyInA = event.filter(this.comparer(this.state.medication_yes));
        var onlyInB = this.state.medication_yes.filter(this.comparer(event));

        todel = onlyInB
       // todel =  this.state.medication_yes.find(x => x.id !== event[0].id)
      } 
      else{
        todel = [this.state.medication_yes[0]]
      }
    }
    catch(err){
      console.log(err)
    ...
    console.log("torem", todel)
    if (todel.length != 0)

    {

      let response = await this.deleteevent(todel)
    ...
    if (isValid) {
      ...
      let responsed = await axios.post(
...

        {
          ...
          Medications:
            this.state.medication === "No" ? "No" : this.state.medication,
      ...
      if (this.state.medication === "Yes") {
        if (
          this.state.medication_yes !== "" ||
          this.state.medication_yes === null
        ) {
        ...
        if (
          this.state.medication_yes_temp !== "" ||
          this.state.medication_yes_temp === null
        ) {
          for (var i = 0; i < this.state.medication_yes_temp.length; i++) {
            if (
              this.state.medication_yes.includes(
                this.state.medication_yes_temp[i]
              )
            ) {
              continue;
            } else {
  ...
  completeForm = async (event) => {
    ...
    await this.apicall(isValid);
    await this.updateworkflow();
    this.changeLoadingCircle(false);

    console.log("kjhsbafojashbfo wjihefiwaehusfijebf");
    // auth.login(() => {
    //       this.props.history.push("/Home");
    //     })
  };
  ...
  validate = () => {
    ...
    if (
      ...
      val.medication === "" ||
      (val.medication === "Yes" &&
        (val.medication_yes === null || val.medication_yes === "")) ||
      val.fracture === "" ||
        ...
        <div className="row has-form-forms">
          ...
          <div>
            <div class="row">
              <div class="col-md-12">
                <div class="form-group">
                  <label className="abc">
                    Do you have any problems with your blood pressure or
                    cholesterol?
                  </label>
                  <label style={{ fontSize: 12, color: "red" }}>
          ...
          <div>
            <div class="row">
              <div class="col-md-12">
                <div class="form-group">
                  <label className="abc">
                    Do you ever have any problems with breathing or being short
                    of breath?{" "}
                  </label>
                  <label style={{ fontSize: 12, color: "red" }}>
                    {this.state.breathing_problems === "" &&
          ...
          <div>
            <div class="row">
              <div class="col-md-12">
                <div class="form-group custom-radio-wrapper">
                  <label className="abc">Do you suffer from headaches? </label>
                  <label style={{ fontSize: 12, color: "red" }}>
                ...
                <div class="col-md-12">
                  <div class="form-group">
                    ...
                    <Select
                      ...
                      isMulti
                      autoFocus
                      onChange={this.onChangeMultipleSelect("medication_yes")}
          ...
          <div>
            <div class="row">
              <div class="col-md-12">
                <div class="form-group custom-radio-wrapper">
                  <label className="abc">
                    Have you ever been very ill eg. fever or spent the night in
                    a hospital
                  </label>
                  <label style={{ fontSize: 12, color: "red" }}>
